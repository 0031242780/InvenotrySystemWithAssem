	...
	private void checkout() {
		...
		try {
			...
			try (Connection con = DBConnection.getConnection()) {
				...
				try {
					...
					for (CartItem c : items) {
						...
						int deductionType = 2;
						StockMovementDAO.logMovementAndUpdateStock(c.getProductId(), c.getQuantity(), deductionType,
								"Stock deducted for Order #" + orderId, account.getAccountId(), con);
					}
			...
			new Alert(Alert.AlertType.INFORMATION,
					"Order created! Total charged: $" + String.format("%.2f", totalAmount)).showAndWait();
